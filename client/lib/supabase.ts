...
export async function testConnection() {
  try {
    ...
    if (tablesError) {
      console.warn(
        "Could not list tables, trying simple select:",
        tablesError.message,
      );
    ...
    if (error) {
      ...
      if (sampleError) {
        ...
        console.log(
          ...
          sampleData?.[0] || "No data",
        );

        // Log expected vs actual columns for easier debugging
        if (tableName === "scores") {
          console.log(
            "Expected scores columns: id, player_name, round, hole_number, strokes",
          );
        } else if (tableName === "contests") {
          console.log(
            "Expected contests columns: id, round, hole_number, winner_name",
          );
        }
      }
    } else {
...
export function transformScoresData(scores: Score[]) {
  ...
  scores.forEach((score) => {
    ...
    if (
      (round === "scarecrow" || round === "thescarecrow") &&
      transformed.scarecrow[playerName]
    ) {
      transformed.scarecrow[playerName][holeIndex] = score.strokes;
    } else if (
      (round === "gamblesands" ||
        round === "gamble sands".replace(/\s+/g, "")) &&
      transformed.gambleSands[playerName]
    ) {
...
export function transformContestData(contests: Contest[]) {
  ...
  contests.forEach((contest) => {
    const round = contest.round
      ...
      .replace(/[^a-z]/g, "");
    const initial =
      playerInitials[contest.winner_name] ||
      contest.winner_name.charAt(0).toUpperCase();
